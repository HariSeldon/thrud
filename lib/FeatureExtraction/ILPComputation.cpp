...
//------------------------------------------------------------------------------
std::vector<unsigned int> getDephts(
  std::map<Instruction *, unsigned int> &inputMap,
  InstVector &insts) {
...
float getILP(BasicBlock *block) {
  ...
  for (InstVector::iterator iter = insts.begin(), end = insts.end();
    ...
    std::vector<unsigned int>::iterator maxPosition = std::max_element(
      currentDepths.begin(), currentDepths.end()); 
    unsigned int max = 0;
