...
bool NDRange::IsGetTid(Instruction *I) {
  bool result = false;
  for (unsigned int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    result |= IsGetTidInDirection(I, direction);
  }
  return result;
}

bool NDRange::IsGetTidInDirection(Instruction *I, unsigned int direction) {
...
bool NDRange::IsGetGlobal(Instruction *I) {
  bool result = false;
  for (unsigned int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    result |= IsGetGlobal(I, direction);
  }
  return result;
}

bool NDRange::IsGetLocal(Instruction *I) {
  bool result = false;
  for (unsigned int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    result |= IsGetLocal(I, direction);
  }
  return result;
}

bool NDRange::IsGetGlobalSize(Instruction *I) {
  bool result = false;
  for (unsigned int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    result |= IsGetGlobalSize(I, direction);
  }
  return result;
}

bool NDRange::IsGetLocalSize(Instruction *I) {
  bool result = false;
  for (unsigned int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    result |= IsGetLocalSize(I, direction);
  }
  return result;
}
...
bool NDRange::IsGetGlobal(Instruction *I, int direction) {
  return IsPresentInDirection(I, GET_GLOBAL_ID, direction);
}

bool NDRange::IsGetLocal(Instruction *I, int direction) {
  return IsPresentInDirection(I, GET_LOCAL_ID, direction);
}

bool NDRange::IsGetGlobalSize(Instruction *I, int direction) {
  return IsPresentInDirection(I, GET_GLOBAL_SIZE, direction);
}

bool NDRange::IsGetLocalSize(Instruction *I, int direction) {
...
void NDRange::FindOpenCLFunctionCallsByNameAllDirs(std::string CalleeName, Function *Caller) {
  for (unsigned int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    std::map<std::string, InstVector> &DirInsts = OCLInsts[direction]; 
...
char NDRange::ID = 0;
static RegisterPass<NDRange>
    X("ndr", "Build NDRange data structure)");
