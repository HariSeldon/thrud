...
cl::opt<int> CoarseningFactor("coarsening-factor", cl::init(2), cl::Hidden,
                              cl::desc("The coarsening factor"));

cl::opt<int> Stride("coarsening-stride", cl::init(1), cl::Hidden,
                    cl::desc("The coarsening stride"));

cl::opt<std::string> KernelName("kernel-name", cl::init(""), cl::Hidden,
...
bool ThreadCoarsening::runOnFunction(Function &F) {
  ...
  for (unsigned int CI = 1; CI < CF; ++CI) {
    ...
    for (unsigned int Index = 0; Index < Regions.size(); ++Index) {
      ...
      RegionBounds NewPair = CloneRegion(
          R->getBounds(), ".." + Twine(CI) + "..", DT, RegionsMap, CIMap);
...
void ThreadCoarsening::InsertSizeScale(unsigned int CD, unsigned int CF,
  ...
  for (InstVector::iterator I = SizeInsts.begin(), E = SizeInsts.end(); I != E;
       ++I) {
    Instruction *Mul = getMulInst(*I, CF);
    Mul->insertAfter(*I);
