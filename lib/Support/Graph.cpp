...
Graph<dataType>::Graph(std::vector<dataType> &nodes) : nodes(nodes), 
...
//------------------------------------------------------------------------------
template <typename dataType>
void Graph<dataType>::modifyGraph(dataType source, dataType dest, bool newValue) {
  unsigned int row = find(source);
  unsigned int column = find(dest);
...
unsigned int Graph<dataType>::find(dataType toFind) {
  typename std::vector<dataType>::iterator element = std::find(nodes.begin(), 
    nodes.end(), toFind);
